...
#pragma mark ORBITALS

- (double)getDistanceFromRSSI:(int16_t)rssi withPowerFunction:(BOOL)usingPowerFunction {
    double distance;
    
    if(usingPowerFunction){
        // using power function
        rssi += 100;
        distance = pow((69.973 / rssi) , 25/8);
    } else {
...
- (void)updateDistanceLabel {
    _distanceLabel.text = [NSString stringWithFormat: @"%ld ft", lroundf(_currentDistance)];
}

- (IBAction)recordRSSI:(id)sender {
    NSLog(@"logging RSSI");
    _logRSSI.text = [NSString stringWithFormat:@"%@, %d", _logRSSI.text, _currentRSSI];
}
